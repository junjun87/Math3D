...
- LLM 自然语言题面解析：DeepSeek / GPT-4o-mini（可配置）
- 拍照 OCR：阿里云教育场景题目识别 + DeepSeek 符号规范化（可设 `OCR_LLM_NORMALIZE=0` 关闭以节省费用）
- 不匹配已注册题型时，自动降级为 LLM 纯文字解题（无 3D 模型）
- 支持随机出题（24 种题型 × 随机参数）
